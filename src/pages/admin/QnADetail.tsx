...
const QnADetail: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    const res = await apiService.deleteQna(Number(id));
    if (res.success) {
      navigate('/admin/qna');
    } else {
      ...
      <div className="container">
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="write-btn" onClick={() => navigate(`/admin/qna/edit/${id}`)}>수정</button>
            <button className="search-btn" onClick={handleDelete}>삭제</button>
            <button className="search-btn" onClick={() => navigate('/admin/qna')}>목록</button>
          </div>
        </div>
      </div>
    </div>
